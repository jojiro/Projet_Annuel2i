 <?php
	...
	if (count($_POST) == 10
		...
		if ($error) {
			...
			$query = $db->prepare("UPDATE users SET gender = :gender, name = :name, surname = :surname, email = :email, birthday = :birthday, country = :country, phone = :phone, situation = :situation, date_updated = :date_updated, is_deleted = :is_deleted, is_admin = :is_admin WHERE id = :id");
		...
		die("Access denied, YOU ARE NOT A ADMIN  : ".$_SERVER["REMOTE_ADDR"]);
	}
